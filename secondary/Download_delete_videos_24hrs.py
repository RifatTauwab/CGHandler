...
import MySQLdb,os,thread,socket
import urllib,time
...
def BuildSrcDest():
    ...
    for asset_id in assetID:
        ...
        message = str(datetime.datetime.now())+" -> %s downloading ..."%(asset_name)
        logging.basicConfig(filename= "E:\casparcg_script\log\download_log\\"+time.strftime("%Y-%m-%d")+'_download.log',level=logging.INFO)
        logging.info(message)
        print message
        DownloadMedia(build_url_src,build_url_dest)
        message = str(datetime.datetime.now())+" -> %s download completed"%(asset_name)
        logging.info(message)
        print message
    db.close()
...
def DeleteFile():
    ...
    while True:
        ...
        for media in AllMedia:
            creation_date = modified_date('E:\CasparCG Server 2.0.7\CasparCG Server\Server\media\%s' %(media))
            if creation_date < (datetime.datetime.now()+ datetime.timedelta(days=-7)):
                ...
                message = str(datetime.datetime.now())+' -> Removing %s' %(media)
                logging.basicConfig(filename= "E:\casparcg_script\log\download_log\\"+time.strftime("%Y-%m-%d")+'_download.log',level=logging.INFO)
...
if __name__ == "__main__":
    ...
    message = str(datetime.datetime.now()) + " -> Download Script Started"
    logging.basicConfig(filename= "E:\casparcg_script\log\download_log\\"+time.strftime("%Y-%m-%d")+'_download.log',level=logging.INFO)
    logging.info(message)
